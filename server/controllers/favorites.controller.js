...
export const getFavorites = async (req, res) => {
  try {
    ...
    const favorites = await Favorites.findAll({
      ...
      include: [
        { model: Recipe, 
          attributes: ["recipe_id", "title", "description", "image_url", "difficulty", "prep_time", "serving", "updatedAt"],
        include: [
          ...
          { model: User, as: "author", attributes: ["user_id", "first_name", "last_name", "avatar"] },
        ]}
      ]
    });
...
export const addFavorite = async (req, res) => {
  try {
    ...
    if(!existRecipe) {
      return res.status(404).json({message: "Not found recipe. You cannot add recipe to favorites because recipe doesn`t found"})
    }
    ...
    console.error(error);
    res.status(500).json({ message: "Server error. Cannot add favorites", error });
  }
};
...
export const deleteFavorite = async (req, res) => {
  try {
    ...
    if(!existRecipe) {
      return res.status(404).json({message: "Not found recipe. You cannot delete recipe to favorites because recipe doesn`t found"})
    }
    ...
    console.error(error);
    res.status(500).json({ message: "Server error. Cannot delete favorites " + error });
  }
};
