...
export const createRecipe = async (req, res) => {
  try {
    ...
    if (!mainImageFile && !image_url) {
      return res
        .status(422)
        .json({ message: "Image is required (file upload or image_url)" });
    }

    const result = await sequelize.transaction(async (t) => {
      // If file uploaded, push to Cloudinary
      if (mainImageFile) {
        const uploaded = await uploadBuffer(mainImageFile.buffer, {
          folder: "recipes",
        });
      ...
      const stepsPayload = [];
      const hasExplicitStepFileMapping =
        Array.isArray(steps) &&
        steps.some(
          (s) => s && typeof s.fileIndex === "number" && s.fileIndex >= 0
        );
      for (let idx = 0; idx < steps.length; idx++) {
        ...
        if (hasExplicitStepFileMapping) {
          if (
            ...
            step.fileIndex >= 0 &&
            step.fileIndex < stepImageFiles.length
          ) {
        ...
        if (!stepImageUrl && stepFile) {
          const uploadedStep = await uploadBuffer(stepFile.buffer, {
        ...
        stepsPayload.push({
          ...
          step_number:
            typeof step.step_number === "number" ? step.step_number : idx + 1,
      ...
      if (stepsPayload.length !== steps.length) {
        return res
          .status(422)
          .json({ message: "Each step must include description" });
      }
      ...
      if (ingredientsPayload.length !== ingredients.length) {
        return res.status(422).json({
          message: "Each ingredient must include name, quantity and unit",
        });
      }

      const createdSteps = await Step.bulkCreate(stepsPayload, {
        transaction: t,
      });
    ...
    return res
      .status(500)
      .json({ message: "Server error. Error create recipe" });
  }
};
  ...
  try {
    ...
    await sequelize.transaction(async (t) => {
      ...
      const shouldDeleteOldImage =
        ...
        oldImagePublicId &&
        image_public_id !== oldImagePublicId;
      ...
      if (steps !== undefined) {
        ...
        // collect old step public ids and urls (so we can reuse images if client didn't send new ones)
        const oldSteps = await Step.findAll({
          where: { recipe_id: recipeId },
          attributes: ["image_public_id", "image_url", "step_number"],
          order: [["step_number", "ASC"]],
          transaction: t,
        });
        // Map old steps by their step_number for convenient reuse
        const oldStepsByNumber = new Map();
        for (const os of oldSteps) {
          const key =
            typeof os.step_number === "number"
              ? os.step_number
              : oldStepsByNumber.size + 1;
          oldStepsByNumber.set(key, {
            image_public_id: os.image_public_id,
            image_url: os.image_url,
          });
        }
        const oldStepPublicIds = new Set(
          oldSteps.map((s) => s.image_public_id).filter(Boolean)
        );

        await Step.destroy({ where: { recipe_id: recipeId }, transaction: t });
        const stepsPayload = [];
        const hasExplicitStepFileMapping = steps.some(
          (s) => s && typeof s.fileIndex === "number" && s.fileIndex >= 0
        );
        for (let idx = 0; idx < steps.length; idx++) {
          const s = steps[idx];
          ...
          // Distinguish between omitted properties and explicit values (including null/empty)
          // If client omitted image fields, we set them to undefined so we can reuse old images.
          const hasImageUrlProp = Object.prototype.hasOwnProperty.call(
            s,
            "image_url"
          );
          const hasImagePublicIdProp = Object.prototype.hasOwnProperty.call(
            s,
            "image_public_id"
          );
          let stepImageUrl = hasImageUrlProp ? s.image_url || null : undefined;
          let stepImagePublicId = hasImagePublicIdProp
            ? s.image_public_id || null
            : undefined;
          ...
          if (hasExplicitStepFileMapping) {
            if (
              typeof s.fileIndex === "number" &&
              s.fileIndex >= 0 &&
              s.fileIndex < stepImageFiles.length
            ) {
          ...
          // If client didn't send image info and there is an old step at same number/index, reuse its image
          // If client didn't send image info (undefined) and there is an old step at same number/index, reuse its image
          if (
            typeof stepImageUrl === "undefined" &&
            typeof stepImagePublicId === "undefined"
          ) {
            const key =
              typeof s.step_number === "number" ? s.step_number : idx + 1;
            const oldStep = oldStepsByNumber.get(key);
            if (oldStep) {
              stepImageUrl = oldStep.image_url || null;
              stepImagePublicId = oldStep.image_public_id || null;
            }
          }

          // Upload file if provided and no image_url already set (or if we want to override reused image)
          if (!stepImageUrl && stepFile) {
            const uploadedStep = await uploadBuffer(stepFile.buffer, {
              folder: "recipes/steps",
            });
        ...
        const newPublicIds = new Set(
          stepsPayload.map((s) => s.image_public_id).filter(Boolean)
        );
        const publicIdsToDelete = [...oldStepPublicIds].filter(
...
export const getMyRecipes = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({
      message: "Server error. Error get recipe all users recipes. " + error,
    });
  }
};
